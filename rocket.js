...
function draw() {
	...
	/* Check if a Rocket is out of bounds */
	var bound = 5000;
	var to_splice = [];
	for (var i = 0; i < Rockets.length ; i++) {
		if ((Rockets[i].pos.x > bound) ||
		    (Rockets[i].pos.x < (0-bound)) ||
		    (Rockets[i].pos.y > bound) ||
		    (Rockets[i].pos.y < (0-bound))) {
			append(to_splice, i);
		}
	}
	for (var i = 0; i < to_splice.length ; i++) {
		Rockets.splice(to_splice[i], 1);
	}
...
function Shooter() {
	...
	this.update = function() {
		...
		if((!this.shooting) && (keyIsPressed)  && (keyCode == ENTER) && (this.c_shooting) && (!this.touchKey)) {
			this.c_shooting = false;
			this.touchKey = true;
			append(Rockets, new Rocket(this.x1, this.y1,this.force_x,this.force_y,this.R,this.G,this.B,10, 20));
		}
		...
		if (!mouseIsPressed && this.shooting) {
			...
			if (this.force > 1) {
				...
				append(Rockets, new Rocket(this.x2, this.y2, (x_dir * this.force_x),(y_dir * this.force_y),this.R,this.G,this.B,10,20));
			}
			this.shooting = false;
		}
	}
	...
	this.draw = function() {
			...
			if ((this.shooting) || (this.c_shooting)) {
				...
				/* Draw the future Rocket */
				noStroke();
                fill(this.R,this.G,this.B);
                translate(this.x2, this.y2);
                
			    rotate( radians(90) + atan2(this.y2-this.y1,this.x2-this.x1) );
